...
void is_malicious(u_char* buf, int size) {
    ...
    for (int i = 0; i < payload_len; i++){
        if (payload[i] == 0x48 && payload[i+1] == 0x6f && payload[i+2] == 0x73 && payload[i+3] == 0x74 && payload[i+4] == 0x3a && payload[i+5] == 0x20){
            start=i+6;
            check++;
        }
        if(check==1){
            for(int i = start; i < payload_len - start; i++){
   ...
   if(jump==1){
      ...
      printf("temp=%s\n",temp);
      string tmp(temp);
      if(malicious_set.find(tmp) != malicious_set.end()){
        printf("URL blocked \n");
        flag=true;
        return;
    }
...
int main(int argc, char **argv)
{
   ...
   ifstream malicious_list;
    malicious_list.open(argv[1]);

    if(!malicious_list.is_open()){
        printf("file open fail \n");
        return -1;
    }
    string tmp;
    while(!malicious_list.eof()){
        malicious_list >> tmp;
        malicious_set.insert(tmp);
    }
    malicious_list.close();
